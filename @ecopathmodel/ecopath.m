...
validateattributes(EM, {'ecopathmodel'}, {'scalar'}, 1);

p = inputParser;
p.addParameter('skipextra', false,  @(x) validateattributes(x, {'logical'}, {'scalar'}));
p.addParameter('debug', false,  @(x) validateattributes(x, {'logical'}, {'scalar'}));
p.addParameter('silent', false,  @(x) validateattributes(x, {'logical'}, {'scalar'}));
p.addParameter('ensemble', zeros(height(EM.pedigree),0), @(x) validateattributes(x, {'numeric'}, {'2d', 'nrows', height(EM.pedigree)})); % TODO
p.parse(varargin{:});
...
multiflag = ~isempty(Opt.ensemble);
...
% Extract relevant data from ecopathmodel object
...
S = struct(...
    ...
    'gs',                      EM.groupdata.gs, ...
    'ba',                      EM.groupdata.ba, ...
    'landing',     table2array(EM.landing), ...
    'discard',     table2array(EM.discard), ...
    'pp',                      EM.groupdata.pp, ...
    'baRate',                  EM.groupdata.baRate, ...
    'emig',                    EM.groupdata.emig, ...
    'emigRate',                EM.groupdata.emigRate, ...
    'immig',                   EM.groupdata.immig, ...
    'dc',          table2array(EM.dc), ...
    'bh',                      EM.groupdata.bh, ...
    'areafrac',                EM.groupdata.areafrac, ...
    'df',          table2array(EM.df), ...
    'discardFate', table2array(EM.discardFate), ...
    'import',                  EM.groupdata.import, ...
    'dtImp',                   EM.groupdata.dtImp);

% If ensemble parameters are included, extract these as well

if multiflag
    
    B = EM.subpedigreevalues(Opt.ensemble);
    
    nens = size(Opt.ensemble, 2);
    
    gflds = {'b','pb','qb','ge','ee','gs','ba','baRate','emig', ...
             'emigRate', 'immig', 'bh', 'areafrac', 'import', 'dtImp', 'pp'};
    if isfield(B, 'groupdata')
        grpdata = permute(num2cell(B.groupdata, 1), [2 3 1]);
        [~, loc] = ismember(gflds, EM.groupdata.Properties.VariableNames);
        
        Ens = cell2struct(grpdata(loc,:), gflds);
        
    else
        Ens = repmat(rmfield(S, setdiff(fieldnames(S), gflds)), nens, 1);
    end
    
    other = {'landing', 'discard', 'dc', 'df', 'discardFate'};
    for io = 1:length(other)
        [Ens(:).(other{io})] = deal(S.(other{io}));
        if isfield(B, other{io})
            for ii = 1:length(Ens)
                Ens(ii).(other{io}) = B.(other{io})(:,:,ii);
            end
        end
    end
    
    [Ens(:).ngroup] = deal(S.ngroup);
    [Ens(:).nlive] = deal(S.nlive);
    [Ens(:).ngear] = deal(S.ngear);
    
end

% Save a copy of the unfilled data
...
e2er   = islive & ~isnan(S.emig)     & ~isnan(S.b) & isnan(S.emigRate);
er2e   = islive & ~isnan(S.emigRate) & ~isnan(S.b) & isnan(S.emig);
ba2bar = islive & ~isnan(S.ba)       & ~isnan(S.b) & isnan(S.baRate);
bar2ba = islive & ~isnan(S.baRate)   & ~isnan(S.b) & isnan(S.ba);
